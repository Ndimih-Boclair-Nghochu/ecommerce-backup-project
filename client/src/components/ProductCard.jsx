    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl">
      <Link to={`/products/${product.id}`} className="block">
              ...
              return (
                <button
                  ...
                  className={`h-10 w-10 flex-shrink-0 overflow-hidden rounded-lg border-2 transition duration-200 ${
                    isActive ? 'border-blue-700 opacity-100' : 'border-gray-200 opacity-70 hover:opacity-100 hover:border-blue-400'
                  }`}
        ...
        <div className="space-y-2 p-3 sm:p-4">
          <p className="text-[11px] font-bold uppercase tracking-wide text-blue-700 line-clamp-1">
          ...
          <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <span className="text-base font-black text-blue-800 sm:text-lg">{formatXAF(product.price)}</span>
          ...
          {!outOfStock && (
            <div className="h-1.5 rounded-full bg-gray-100">
              <div className="h-1.5 rounded-full bg-blue-700" style={{ width: `${Math.min(stock, 100)}%` }} />
            </div>
          )}
        </div>
      </Link>
      ...
      <div className="mt-auto grid grid-cols-[1fr_42px] gap-2 px-3 pb-3 sm:px-4 sm:pb-4">
        <button
          ...
          className="rounded-xl bg-blue-700 px-2 py-2 text-xs font-black text-white transition hover:bg-blue-800 disabled:bg-gray-300 disabled:text-gray-500 sm:text-sm"
        >
          {t('addToCart')}
        </button>
        <button
