...
function ProductCard({ product, addToCart, toggleWishlist, isInWishlist }) {
  ...
  return (
    <article className="group rounded-lg border border-gray-200 bg-white shadow-sm transition hover:shadow-md overflow-hidden">
      ...
      <div className="px-4 pb-4 grid grid-cols-[1fr_44px] gap-2">
        ...
        <button
          ...
          className={`rounded-md border text-lg ${isInWishlist(product.id) ? 'border-red-500 bg-red-500 text-white' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
    ...
    <main className="min-h-screen bg-gray-50">
      <section className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-gray-950">All Products</h1>
              <p className="mt-2 text-gray-600">Showing {filteredProducts.length} of {products.length} products</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                className="rounded-md border border-gray-300 px-3 py-2 focus:border-blue-700 focus:outline-none"
              />
              <select
                value={sortBy}
              ...
              <button
                ...
                className="md:hidden rounded-md border border-gray-300 px-4 py-2 font-semibold"
              >
            ...
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Array.from({ length: 9 }).map((_, index) => (
                  <div key={index} className="h-80 rounded-lg bg-white border border-gray-200 animate-pulse" />
                ))}
              </div>
              ...
              <div className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center">
                <h2 className="text-xl font-bold text-gray-950">No products found</h2>
                <p className="mt-2 text-gray-600">Try changing your search or clearing filters.</p>
      ...
      <div className={`fixed inset-0 z-50 md:hidden ${filtersOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        <button
          ...
          className={`absolute inset-0 bg-gray-950/50 transition-opacity ${filtersOpen ? 'opacity-100' : 'opacity-0'}`}
        />
        <div className={`absolute inset-x-0 bottom-0 rounded-t-2xl bg-white p-5 shadow-2xl transition-transform duration-300 ${filtersOpen ? 'translate-y-0' : 'translate-y-full'}`}>
          <div className="mb-4 flex items-center justify-between">
            ...
            <button type="button" onClick={() => setFiltersOpen(false)} className="rounded-md border border-gray-300 px-3 py-2 text-xl leading-none">×</button>
          </div>
          <Filters categories={categories} filters={filters} setFilters={setFilters} activeFilterCount={activeFilterCount} clearFilters={clearFilters} />
