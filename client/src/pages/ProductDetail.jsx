    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 sm:px-6 py-8">
        <div className="max-w-7xl mx-auto grid gap-6 lg:grid-cols-2">
          <div className="aspect-square rounded-lg bg-white border border-slate-200 animate-pulse" />
          <div className="space-y-4">
            <div className="h-10 rounded bg-white border border-slate-200 animate-pulse" />
            <div className="h-6 w-40 rounded bg-white border border-slate-200 animate-pulse" />
            <div className="h-32 rounded bg-white border border-slate-200 animate-pulse" />
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main className="bg-slate-50 min-h-screen">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-2">
              ...
              <div className="mt-4 grid grid-cols-4 sm:grid-cols-6 gap-3">
                {variants.map((variant) => (
                  <button
                    ...
                    className={`aspect-square overflow-hidden rounded-md border-2 ${selectedVariant?.url === variant.url ? 'border-blue-700' : 'border-slate-200'}`}
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5 sm:p-6">
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {product.isNew && <span className="rounded-full bg-amber-400 px-3 py-1 text-xs font-bold text-slate-900">New</span>}
              {product.mostOrdered && <span className="rounded-full bg-blue-700 px-3 py-1 text-xs font-bold text-white">Popular</span>}
            </div>

            <h1 className="text-3xl sm:text-4xl font-bold text-slate-950">{product.name}</h1>
            ...
            <div className="mt-6">
              ...
              <p className="mt-2 text-slate-600 leading-relaxed">{product.description || 'No description provided yet.'}</p>
            </div>

            {variants.length > 1 && (
              <div className="mt-6">
                <h2 className="font-bold text-slate-950 mb-3">Variant</h2>
                <div className="flex flex-wrap gap-2">
                  {variants.map((variant) => (
                    <button
                      ...
                      className={`rounded-md border px-4 py-2 text-sm font-semibold ${selectedVariant?.url === variant.url ? 'border-blue-700 bg-blue-50 text-blue-800' : 'border-slate-300 text-slate-700'}`}
                    >
            ...
            <div className="mt-6 flex items-center gap-3">
              <button
                ...
                className="h-11 w-11 rounded-md border border-slate-300 text-xl font-bold"
              >
                −
              </button>
              <input
                ...
                className="h-11 w-20 rounded-md border border-slate-300 text-center font-bold"
              />
              <button
                type="button"
            ...
            <div className="mt-6 grid grid-cols-[1fr_56px] gap-3">
              <button
                ...
                className="rounded-md bg-blue-700 px-5 py-3 font-bold text-white hover:bg-blue-800 disabled:bg-slate-300 disabled:text-slate-500"
              >
                Add to Cart
              </button>
              <button
                ...
                className={`rounded-md border text-2xl ${isInWishlist(product.id) ? 'border-red-500 bg-red-500 text-white' : 'border-slate-300 text-slate-700 hover:bg-slate-50'}`}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-12">
        <h2 className="text-2xl font-bold text-slate-950 mb-4">Related Products</h2>
        {related.length ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {related.map((item) => (
              <Link key={item.id} to={`/products/${item.id}`} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm hover:shadow-md transition">
                <img src={getProductImage(item)} alt={item.name} className="aspect-[4/3] w-full rounded-md object-cover bg-slate-100" />
                <h3 className="mt-3 font-bold text-slate-950 line-clamp-2">{item.name}</h3>
                <p className="mt-1 text-blue-800 font-bold">{formatXAF(item.price)}</p>
              </Link>
            ))}
          </div>
        ) : (
          <p className="rounded-lg border border-slate-200 bg-white p-5 text-slate-600">No related products yet.</p>
        )}
      </section>
    </main>
  )
